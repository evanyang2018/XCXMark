...
public class ImgUtil {
    ...
    public static void main(String[] arg) throws IOException {
        ...
        System.out.println("输入的图片大小：" + mypic.getPicSize("f:\\222.png") / 1024 + "KB");
        int count = 0; // 记录全部图片压缩所用时间
        for (int i = 0; i < 1; i++) {
            int start = (int) System.currentTimeMillis(); // 开始时间
            BufferedImage bufferedImage = ImageIO.read(new File("f:\\222.png"));
            int width = bufferedImage.getWidth();
            int height = bufferedImage.getHeight();
            System.out.println("宽度>>" + width + "   高度>>" + height);
            mypic.compressPic("f:\\", "f:\\", "222.png", "222.png", width, height, true);
            int end = (int) System.currentTimeMillis(); // 结束时间
            int re = end - start; // 但图片生成处理时间
            count += re;
            System.out.println("第" + (i + 1) + "张图片压缩处理使用了: " + re + "毫秒");
            System.out.println("输出的图片大小：" + mypic.getPicSize("f:\\test\\r1" + i + ".png") / 1024 + "KB");
        }
        System.out.println("总共用了：" + count + "毫秒");
    }

    // 图片处理
    public String compressPic(String imgUrl) {
        logger.info("要压缩的》》图片地址>>>>" + imgUrl);
        try {
            // 获得源文件
